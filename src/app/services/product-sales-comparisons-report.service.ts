...
export class ProductSalesComparisonsReportService implements OnDestroy {
  ...
  generateReport(
    ...
    forkJoin([
      this.orderService.generateProductSalesComparisonsReport(input),
  ...
  private generateProductSalesComparisonsReportPdf(
    ...
    const fileName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:DownloadableFileName',
    );
    doc.save(fileName);
  }
  ...
  private addReportContents(doc: jsPDF, body: any, logoBase64: string, currentTime: string) {
    ...
    const productInfoColumnName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:ProductInfo',
    );
    const productCodeColumnName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:ProductCode',
    );
    const productDescriptionColumnName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:ProductDescription',
    );
    ...
    const unitsColumnName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:Units',
    );
    const avgColumnName = this.localizationService.instant('::Reports:ProductSalesComparisons:Avg');
    ...
    const reportName = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:ReportName',
    );
    const devColumnName = this.localizationService.instant('::Reports:ProductSalesComparisons:Dev');
    const difference = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:Difference',
    );
    ...
    autoTable(doc, {
      ...
      head: [
        [
          { content: productInfoColumnName, colSpan: 2 },
  ...
  private getReportBody(orders: SubOrderControlListDto[]) {
    ...
    const finalGroupOrders = orders.reduce(
      (grouped, order) => {
        if (!grouped[order.productCode]) {
          grouped[order.productCode] = [];
        }
        grouped[order.productCode].push(order);
        return grouped;
      },
    ...
    const bodyContents = Object.keys(finalGroupOrders).map(productCode => {
      const firstSet = firstSetOrders[productCode] || {
        totalUnits: 0,
        totalUnitPrice: 0,
        avgUnitPrice: 0,
      };
      const secondSet = secondSetOrders[productCode] || {
        totalUnits: 0,
        totalUnitPrice: 0,
        avgUnitPrice: 0,
      };
      ...
      const diffAvgPrice = secondSet.totalUnitPrice
        ? (diffUnitPrice / secondSet.totalUnitPrice) * 100
        : diffUnitPrice !== 0
          ? 100
          : 0;
      ...
      return [
        productCode,
        finalGroupOrders[productCode][0]?.productDescription || '',
        firstSet.totalUnitPrice.toFixed(2),
        firstSet.totalUnits,
        firstSet.avgUnitPrice.toFixed(2),
        secondSet.totalUnitPrice.toFixed(2),
        secondSet.totalUnits,
        secondSet.avgUnitPrice.toFixed(2),
        diffUnitPrice.toFixed(2),
        diffUnits,
  ...
  private addLocationTotal(
    ...
    const locationTotalText = this.localizationService.instant(
      '::Reports:ProductSalesComparisons:SubTotals',
    );
    ...
    const diffAvgUnitPrice = secondTotalUnitPrice
      ? (diffTotalUnitPrice / secondTotalUnitPrice) * 100
    ...
    bodyContents.push([
      '',
      locationTotalText,
      firstTotalUnitPrice.toFixed(2),
      firstTotalUnits,
      avgFirstUnitPrice.toFixed(2),
      secondTotalUnitPrice.toFixed(2),
      secondTotalUnits,
      avgSecondUnitPrice.toFixed(2),
      diffTotalUnitPrice.toFixed(2),
      diffTotalUnits,
  ...
  private filterAndGroupOrders(orders: SubOrderControlListDto[], dateRange: DateRangeType) {
    return orders
      ...
      .reduce(
        (grouped, order) => {
          const { productCode, totalUnitPrice, totalUnits } = order;
          if (!grouped[productCode]) {
            grouped[productCode] = { ...order };
          } else {
            grouped[productCode].totalUnitPrice += totalUnitPrice;
            grouped[productCode].totalUnits += totalUnits;
            grouped[productCode].avgUnitPrice =
              grouped[productCode].totalUnitPrice / grouped[productCode].totalUnits;
          }
          return grouped;
        },
        {} as Record<string, SubOrderControlListDto>,
