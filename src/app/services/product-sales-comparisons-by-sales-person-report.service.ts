...
export class ProductSalesComparisonsBySalesPersonReportService implements OnDestroy {
  ...
  selectedEmployeeClassNames: string[] = [];
  ...
  generateReport(
    ...
    this.selectedEmployeeClassNames = selectedEmployeeClassNames;

    forkJoin([
      this.salesComparisonsReportService.generateProductSalesComparisonsBySalesPersonReport(input),
      ...
      .subscribe(([orders, logoBlob]) => {
        ...
        reader.onloadend = () => {
          ...
          this.generateProductSalesComparisonsBySalesPersonReportPdf(orders, logoBase64);
        };
      });
  }

  private generateProductSalesComparisonsBySalesPersonReportPdf(
    ...
    if (this.reportCategory === ReportCategory.Details) {
      ...
      const salesRepOrderEntries = Object.entries(this.groupOrdersBySalesRep(orders));
      ...
      salesRepOrderEntries.forEach(([salesRep, salesRepOrders], index) => {
        ...
        this.addReportContents(doc, reportBody, logoBase64, currentTime, salesRepName);
    ...
    const fileNameText =
      this.reportCategory === ReportCategory.Summary
        ? '::Reports:ProductSalesComparisonsBySalesPerson:Summary:DownloadableFileName'
        : '::Reports:ProductSalesComparisonsBySalesPerson:Details:DownloadableFileName';
    const fileName = this.localizationService.instant(fileNameText);
    doc.save(fileName);
  }
  ...
  private addReportContents(
    ...
    const reportName = this.localizationService.instant(
      '::Reports:ProductSalesComparisonsBySalesPerson:ReportName',
    );
    const noEmployeeText = this.localizationService.instant(
      '::Reports:ProductSalesComparisonsBySalesPerson:NoEmployee',
    );

    const offSet = this.reportCategory === ReportCategory.Details ? 41 : 33;
    ...
    if (this.reportCategory === ReportCategory.Details) {
      ...
      doc.setFontSize(14);
      doc.text(`${salesPersonFilter} ${salesRepName || noEmployeeText}`, 14, mainTableY - 5);
    }
  ...
  private getTableHeaders(): { firstHeaderRow: any[]; secondHeaderRow: string[] } {
    const salesPersonColumnName = this.localizationService.instant(
      '::Reports:ProductSalesComparisonsBySalesPerson:SalesPerson',
    );
    ...
    const firstHeaderRow =
      this.reportCategory === ReportCategory.Summary
        ? [
            { content: salesPersonColumnName, colSpan: 1 },
  ...
  private groupOrdersBySalesRep(orders: SubOrderControlListDto[]): {
  ...
  private addLocationTotal(
    ...
    const salesRep = salesRepName ? `*** ${salesRepName}  TOTALS ***` : noEmployeeText;
    const locationTotalText =
      this.reportCategory === ReportCategory.Summary ? locationTotalTextTemplate : salesRep;
  ...
  getGroupedKey(order: SubOrderControlListDto) {
    return `${order.salesRepresentativeId}${
  ...
  private getFilterTableBody(): any[][] {
    ...
    const employeeClasses = formatFilterItem(this.selectedEmployeeClassNames, allSelectionText);
    ...
    const employeeClassFilter = this.localizationService.instant('::Reports:Filters:EmployeeClass');
    ...
    return [
      ...
      [employeeClassFilter, employeeClasses],
    ];
  }
